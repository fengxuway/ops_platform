...
class ResultsCollector(CallbackBase):
    ...
    def v2_runner_on_unreachable(self, result):
        ...
        print("[%s] [%s] %s" % (task_name, ip, {'status': 'unreachable', 'result': result._result}))
    ...
    def v2_runner_on_ok(self, result,  *args, **kwargs):
        ...
        print("[%s] [%s] %s" % (task_name, ip, {'status': 'ok', 'result': result._result}))
    ...
    def v2_runner_on_failed(self, result,  *args, **kwargs):
        ...
        print("[%s] [%s] %s" % (task_name, ip, {'status': 'failed', 'result': result._result}))

    def v2_playbook_on_task_start(self, task, is_conditional):
        # 每次调用该方法表示已执行完一个任务(除第一个任务外, 用self.switch标记, True表示已经存在任务),
        # 使用self.current_task缓存上一个任务
        if not self.switch:
            self.switch = True
        else:
            self.v2_playbook_on_task_complete()
    ...
    def v2_playbook_on_complete(self):
        # 执行完playbook后手动执行该方法
        self.v2_playbook_on_task_complete()
        pass
...
class Runner(object):
    ...
    class Task(dict):
        def __init__(self, name, module_name, module_args=None, register='', **kwargs):
            ...
            # self['task_id'] = kwargs.get('_id', None)
            # self.task_id = self['task_id']
            # self['super_task_name'] = kwargs.get('super_task_name', None)
    ...
    def create_play_tasks(self):
        if not self.playbook:
            return Runner.Playbook(name=self.task_name, task_list=self.get_task_list(), pattern=self.patterns,
...
"""
    ...
    def run(self, callback=None):
        ...
        playbook = self.create_play_tasks()
        ...
        try:
            ...
            result = tqm.run(play)
            if isinstance(callback, ResultsCollector):
                callback.v2_playbook_on_complete()
        finally:
            if tqm is not None:
                tqm.cleanup()
...
def main():
    # 定义playbook任务列表
    task_list = [Runner.Task('task1', 'shell', 'ls ~'),
                 Runner.Task('task2', 'shell', 'ls /')]

    # 定义playbook对象
    p2 = Runner.Playbook(task_list=task_list)

    # 执行playbook
    runner = Runner(playbook=p2)
    # 执行单个任务或多个任务
    # runner = AnsibleRunner(task_list=task_list[0])
    # runner = AnsibleRunner(task_list=task_list)
    code, result = runner.run()
    for task_name, t in list(result.items()):
        print(task_name)
        for ip, r in list(t.items()):
            print("\t", ip)
            print('\t' * 2, r)
